    ...
    namespace hal {
        ...
        namespace spi {
            ...
            const SpiPort spiPort[SPI_PORT_COUNT] = {
                    ...
                    {{B, PIN15}, {B, PIN14}, {B, PIN13}, AlternateFunction::AF5}, /* SPI2 MOSI, MISO, SCK */
                    {{F, PIN9}, {F, PIN8}, {F, PIN7}, AlternateFunction::AF5}  /* SPI5 MOSI, MISO, SCK */
            };


            pegasus::hal::SpiDriver spi1(SPI1, spiPort[SPI_1]);
            pegasus::hal::SpiDriver spi2(SPI2, spiPort[SPI_2]);
        }
        ...
        namespace uart {
            ...
            const UartConfig uartConfig[UART_PORT_COUNT] = {
                    {{A, PIN10},{A, PIN9}, AlternateFunction::AF7, 115200}, /* UART 1 TELEM / FLASH */
                    {{D, PIN6},{D, PIN5}, AlternateFunction::AF7, 9600},  /* UART 2 GPS */
                    {{B, PIN11},{B, PIN10}, AlternateFunction::AF7, 115200}   /* UART 3 GPS EXT */
            };
...
pegasus::peripherals::Led ledGreen(E, PIN2);
pegasus::peripherals::Led ledBlue(E, PIN3);
/*pegasus::peripherals::Led ledBlue(E, PIN3);*/

pegasus::hal::Gpio baroCSPin(E, PIN15);
pegasus::hal::SpiDevice spiBARO(&pegasus::hal::spi::spi2, &baroCSPin);
pegasus::peripherals::MS5611 MS5611(&spiBARO);

/* GPS */
pegasus::peripherals::UBlox ublox(&uart3);
...
void initPlatform()
{
    ...
    pegasus::core::com.addDevice(&pegasus::hal::uart::uart1); // Telemetry

    /* Platform information */
    Clocks clocks;
    Core::getSystemClock(&clocks);

    pegasus::core::trace.log("Platform:\tPegasus STM32V2B");
    ...
    pegasus::fc::engine.addLed(LED_YELLOW, &ledGreen);
    ...
    /* INIT Engine */
    pegasus::fc::engine.init(&tim6, &pegasus::fc::rc, &mpu6000, &MS5611, &sonar, &ublox);
}
