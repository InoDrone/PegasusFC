        ...
        namespace stm32f4 {
            ...
            class Processor {
                public:
                    ...
                    /* Context */
                    static void saveContext(uint32_t** stack);
                    static void loadContext(uint32_t** stack);
            };
            ...
            inline
            __attribute__((naked))
            void Processor::saveContext(uint32_t** pointer)
            {
                asm volatile (
                  "ldr   r1, [r0]               \n"
                  "                             \n"
                  "mrs   r2, PSP                \n"  // Save PSP to r1
                  "stmdb r2!, {r4-r11}          \n"  // Save PSP and R4-R11 to pointer
                  //"add r1, r1, #0x20            \n"
                  "str   r2, [r1]               \n"
            ...
            inline
            __attribute__((naked))
            void Processor::loadContext(uint32_t** pointer)
            {
                asm volatile (
                  "ldr   r1, [r0]               \n"  // Load pointer addresse
                  "                             \n"
                  "ldmia r1!, {r4-r11}          \n"  // Load pointer to r1, r4-r11
                  //"add r1, r1, #0x20            \n"
                  "msr   PSP, r1                \n"  // Load r1 to PSP
                  "                             \n"
                  "bx lr                        \n"  // Return function
