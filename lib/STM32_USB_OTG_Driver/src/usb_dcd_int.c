...
uint32_t USBD_OTG_EP1OUT_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if ( doepint.b.xfercompl )
  {
    ...
    /* RX COMPLETE */
    USBD_DCD_INT_cb.DataOutStage(pdev , 1);
...
uint32_t USBD_OTG_EP1IN_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if ( diepint.b.xfercompl )
  {
    ...
    /* TX COMPLETE */
    USBD_DCD_INT_cb.DataInStage(pdev , 1);
  }
  if ( diepint.b.ahberr )
  {
...
{
  USB_OTG_GINTSTS_TypeDef  gintsts;  
  USBD_DCD_INT_cb.DevConnected (pdev);
...
static uint32_t DCD_OTG_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  {
    USBD_DCD_INT_cb.DevDisconnected (pdev);
  }
  /* Clear OTG interrupt */
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) /* In ITR */
    {
      ...
      if ( diepint.b.xfercompl )
      {
        ...
        /* TX COMPLETE */
        USBD_DCD_INT_cb.DataInStage(pdev , epnum);
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1)
    {
      ...
      if ( doepint.b.xfercompl )
      {
        ...
        /* Inform upper layer: data ready */
        /* RX COMPLETE */
        USBD_DCD_INT_cb.DataOutStage(pdev , epnum);
      ...
      if ( doepint.b.setup )
      {
        ...
        /* SETUP COMPLETE */
        USBD_DCD_INT_cb.SetupStage(pdev);
...
static uint32_t DCD_IsoINIncomplete_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  USBD_DCD_INT_cb.IsoINIncomplete (pdev);
...
static uint32_t DCD_IsoOUTIncomplete_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  USBD_DCD_INT_cb.IsoOUTIncomplete (pdev);
